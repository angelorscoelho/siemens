<template>
  <div
    class="mt-2 px-3 py-2.5 rounded-lg text-xs leading-snug relative flex items-start gap-2 cursor-pointer transition-colors"
    :class="bannerClass"
  ...
  >
    ...
    <div class="flex-1 min-w-0">
      <p class="font-medium leading-snug" :class="titleClass">{{ title }}</p>
      <p v-if="subtitle" class="text-gray-400 leading-snug mt-0.5">{{ subtitle }}</p>
      <p v-if="hint" class="text-gray-500 leading-snug mt-0.5 italic">{{ hint }}</p>
    </div>
