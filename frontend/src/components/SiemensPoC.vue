    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      <transition name="slide">
        <div v-if="assistantOpen"
          ...
          <div ref="chatScrollRef" class="flex-1 overflow-y-auto p-4 space-y-3">
            <div v-if="!API_URL" class="bg-yellow-900 bg-opacity-30 border border-yellow-700 rounded-xl px-3 py-2.5 text-xs text-yellow-300 flex items-start gap-2">
              <svg class="w-3 h-3 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <span>AI Assistant is not connected. Set <code class="font-mono bg-yellow-900 bg-opacity-50 px-1 rounded">VITE_API_URL</code> in your <code class="font-mono bg-yellow-900 bg-opacity-50 px-1 rounded">.env</code> file and rebuild to enable it.</span>
            </div>
          ...
          <div class="border-t border-gray-700 p-3">
            <form @submit.prevent="sendMessage" class="flex gap-2">
              <input
                v-model="inputText"
                type="text"
                :placeholder="API_URL ? 'Ask about maintenance…' : 'AI Assistant not configured'"
              ...
              <button
                type="submit"
                :disabled="loading || !inputText.trim() || !API_URL"
      ...
      <div v-if="mobileView === 'chat'"
        ...
        <div ref="mobileChatScrollRef" class="flex-1 overflow-y-auto p-4 space-y-3">
          <div v-if="!API_URL" class="bg-yellow-900 bg-opacity-30 border border-yellow-700 rounded-xl px-4 py-3 text-sm text-yellow-300 flex items-start gap-2">
